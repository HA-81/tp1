...
public class Image extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * Récupération du chemin du fichier demandé au sein de l'URL de la requète
		 */
		...
		if (!fichier.exists()) {
			/*
			 * Si non, alors on evoie une erreur 404, qui signifie que la ressource demandée
			 * n'existe pas
			 */
			response.sendError(HttpServletResponse.SC_NOT_FOUND);
			return;
		...
		/*
		 * Si le type de fichier est inconnu, alors on initialise un type par default
		 */
		if (type == null) {
